...
const T = {
  ...
  chalk: "#EEE9E3", dim: "#7A8FA8",
};

const FAMILY_CATS = [
  { id: "hogar", label: "Hogar", icon: "🏡", color: T.blue, subs: ["Alquiler/Hipoteca", "Luz", "Agua", "Gas", "Internet", "Comunidad"] },
  { id: "alimentacion", label: "Alimentación", icon: "🛒", color: T.green, subs: ["Supermercado", "Mercado", "Alimentación bebés"] },
  { id: "transporte", label: "Transporte", icon: "🚗", color: T.amber, subs: ["Gasolina", "Transporte público", "Parking", "Mantenimiento coche"] },
  { id: "educacion", label: "Educación", icon: "🎓", color: T.purple, subs: ["Colegio/Universidad", "Extraescolares", "Libros/Material", "Clases particulares"] },
  { id: "salud", label: "Salud", icon: "❤️", color: T.pink, subs: ["Médico/Farmacia", "Seguro salud", "Dentista", "Gimnasio"] },
  { id: "seguros", label: "Seguros", icon: "🛡", color: T.teal, subs: ["Seguro hogar", "Seguro coche", "Seguro vida"] },
  { id: "ocio", label: "Ocio", icon: "🎉", color: T.red, subs: ["Restaurantes", "Cine/Teatro", "Vacaciones", "Suscripciones"] },
  { id: "deudas", label: "Deudas", icon: "💳", color: "#FF9500", subs: ["Préstamo personal", "Tarjeta crédito", "Otros préstamos"] },
  { id: "ahorro", label: "Ahorro", icon: "💰", color: T.green, subs: ["Fondo emergencia", "Ahorro vacaciones", "Plan pensiones"] },
  { id: "otros", label: "Otros", icon: "📦", color: T.dim, subs: ["Ropa", "Regalos", "Mascotas", "Otros"] },
];

function generateProfessionalFiscalPDF(tipo, datos) {
  const { ingresos, gastos } = datos;
  const margen = ((ingresos - gastos) / ingresos * 100).toFixed(1);
  const ivaAIngresar = ((ingresos - gastos) * 0.21).toFixed(2);
  const irpfAIngresar = ((ingresos - gastos) * 0.20).toFixed(2);

  let txt = \`
╔══════════════════════════════════════════════════════════════════╗
║          DECLARACIÓN FISCAL PROFESIONAL - ContaAI v3            ║
...
═══════════════════════════════════════════════════════════════════
INFORME TRIMESTRAL - \${new Date().toLocaleDateString("es-ES")}
═══════════════════════════════════════════════════════════════════

RESUMEN FINANCIERO:
  • Ingresos totales ....................... \${ingresos.toLocaleString("es-ES")}€
  • Gastos totales ......................... \${gastos.toLocaleString("es-ES")}€
  • Beneficio neto ......................... \${(ingresos - gastos).toLocaleString("es-ES")}€
  • Margen real ............................ \${margen}%
  • Eficiencia operativa ................... \${(100 - (gastos / ingresos) * 100).toFixed(1)}%

Generado por ContaAI v3
\`;
...
const CSS = \`
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .hamburger span{display:block;width:22px;height:2px;background:#F5A623;border-radius:2px;transition:all .3s}
\`;
...
function Btn({ children, onClick, variant = "primary", style = {}, full, disabled }) {
...
function Input({ label, value, onChange, type = "text", placeholder }) {
  return (<div style={{ marginBottom: 14 }}>{label && <label style={{ fontSize: 11, color: T.dim, fontWeight: 600, textTransform: "uppercase", display: "block", marginBottom: 5 }}>{label}</label>}<input type={type} value={value} onChange={onChange} placeholder={placeholder} style={{ width: "100%", padding: "10px 12px", background: T.navyLight, border: \`1px solid \${T.border}\`, borderRadius: 10, color: T.chalk, fontSize: 14 }} onFocus={(e) => (e.target.style.borderColor = T.amber)} onBlur={(e) => (e.target.style.borderColor = T.border)} /></div>);
}

function Sel({ label, value, onChange, options }) {
  return (<div style={{ marginBottom: 14 }}>{label && <label style={{ fontSize: 11, color: T.dim, fontWeight: 600, textTransform: "uppercase", display: "block", marginBottom: 5 }}>{label}</label>}<select value={value} onChange={onChange} style={{ width: "100%", padding: "10px 12px", background: T.navyLight, border: \`1px solid \${T.border}\`, borderRadius: 10, color: T.chalk, fontSize: 14 }}>{options.map((o) => (<option key={o.value} value={o.value}>{o.label}</option>))}</select></div>);
}
...
function StatCard({ label, value, sub, color = T.amber, icon }) {
  return (<Card><div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}><div><div style={{ color: T.dim, fontSize: 11, fontWeight: 600, marginBottom: 7, textTransform: "uppercase" }}>{label}</div><div className="mono" style={{ fontSize: 22, fontWeight: 600, color, marginBottom: 3 }}>{value}</div>{sub && <div style={{ color: T.dim, fontSize: 12 }}>{sub}</div>}</div>{icon && <div style={{ fontSize: 22, opacity: 0.7 }}>{icon}</div>}</div></Card>);
}
...
function BusinessDashboard({ session, showToast }) {
  ...
  const horasTrabajadas = 120;
  const ingresosPorHora = (totalInc / horasTrabajadas).toFixed(2);
  ...
  return (
    <div className="fu" style={{ padding: "24px 20px" }}>
      ...
      <Card glow={T.blue + "33"} style={{ marginBottom: 16 }}>
        <div className="serif" style={{ fontSize: 16, marginBottom: 14, color: T.blue }}>📊 Métricas Avanzadas Autónomo</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
          <div><div style={{ color: T.dim, fontSize: 11, marginBottom: 6 }}>Ingreso por hora real</div><div className="mono" style={{ fontSize: 18, color: T.green, fontWeight: 700 }}>{ingresosPorHora}€</div><div style={{ fontSize: 10, color: T.dim }}>con {horasTrabajadas}h/mes</div></div>
          <div><div style={{ color: T.dim, fontSize: 11, marginBottom: 6 }}>Ratio gasto/ingreso</div><div className="mono" style={{ fontSize: 18, color: ((totalExp / totalInc) * 100) < 50 ? T.green : T.amber, fontWeight: 700 }}>{((totalExp / totalInc) * 100).toFixed(0)}%</div><div style={{ fontSize: 10, color: T.dim }}>vs ideal 40%</div></div>
          <div><div style={{ color: T.dim, fontSize: 11, marginBottom: 6 }}>Eficiencia operativa</div><div className="mono" style={{ fontSize: 18, color: T.blue, fontWeight: 700 }}>{(100 - (totalExp / totalInc) * 100).toFixed(0)}%</div><div style={{ fontSize: 10, color: T.dim }}>margen bruto</div></div>
        </div>
      </Card>

      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
        <Btn full onClick={() => { generateProfessionalFiscalPDF("303", { ingresos: totalInc, gastos: totalExp }); showToast("Modelo 303 descargado ✓"); }} style={{ background: \`linear-gradient(135deg,\${T.amber},\${T.amberL})\`, color: T.navy }}>📄 Descargar Modelo 303</Btn>
        <Btn full onClick={() => { generateProfessionalFiscalPDF("130", { ingresos: totalInc, gastos: totalExp }); showToast("Modelo 130 descargado ✓"); }} style={{ background: \`linear-gradient(135deg,\${T.blue},#5BA8FF)\`, color: T.white }}>📋 Descargar Modelo 130</Btn>
      </div>
...
function FamilyDashboard({ session, showToast }) {
  ...
  const categoryTrend = expenses.reduce((acc, e) => { acc[e.cat] = (acc[e.cat] || 0) + (e.amount || 0); return acc; }, {});
  const maxCat = Object.entries(categoryTrend).sort((a, b) => b[1] - a[1])[0];
  ...
  return (
    <div className="fu" style={{ padding: "24px 20px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 20 }}>
        <StatCard label="Ingresos mes" value={\`\${totalInc.toLocaleString("es-ES")}€\`} color={T.green} icon="💼" />
        <StatCard label="Gastos mes" value={\`\${totalExp.toLocaleString("es-ES")}€\`} color={T.red} icon="🛒" />
      ...
      <Card glow={T.teal + "33"} style={{ marginBottom: 16 }}>
        <div className="serif" style={{ fontSize: 16, marginBottom: 14, color: T.teal }}>👨‍👩‍👧‍👦 Análisis Familia Avanzado</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
          <div><div style={{ color: T.dim, fontSize: 11, marginBottom: 6 }}>Índice de ahorro</div><div className="mono" style={{ fontSize: 18, color: ahorroIndex > 20 ? T.green : T.amber, fontWeight: 700 }}>{ahorroIndex}%</div><div style={{ fontSize: 10, color: T.dim }}>Target: 20%</div></div>
          <div><div style={{ color: T.dim, fontSize: 11, marginBottom: 6 }}>Categoría top</div><div style={{ fontSize: 13, fontWeight: 600 }}>{maxCat ? FAMILY_CATS.find(c => c.id === maxCat[0])?.label : "N/A"}</div><div style={{ fontSize: 10, color: T.dim }}>{maxCat ? maxCat[1].toLocaleString("es-ES") : "0"}€</div></div>
          <div><div style={{ color: T.dim, fontSize: 11, marginBottom: 6 }}>Predicción 5 años</div><div className="mono" style={{ fontSize: 16, color: T.green, fontWeight: 700 }}>{(saldo * 12 * 5).toLocaleString("es-ES")}€</div><div style={{ fontSize: 10, color: T.dim }}>ahorro acumulado</div></div>
        </div>
      </Card>

      <Card><div className="serif" style={{ fontSize: 16, marginBottom: 12 }}>💡 Regla 50/30/20</div><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 14 }}>
        <div><div style={{ fontSize: 12, color: T.dim, marginBottom: 4 }}>Ahorro acumulado</div><div className="mono" style={{ fontSize: 20, color: T.green, fontWeight: 700 }}>{(saldo * 12 * 5).toLocaleString("es-ES")}€</div></div>
        <div><div style={{ fontSize: 12, color: T.dim, marginBottom: 4 }}>Con ahorro mensual</div><div className="mono" style={{ fontSize: 18, color: T.teal, fontWeight: 600 }}>{saldo.toLocaleString("es-ES")}€</div></div>
      </div><div style={{ background: T.teal + "12", border: \`1px solid \${T.teal}33\`, borderRadius: 10, padding: 12 }}><div style={{ fontSize: 12, color: T.teal, fontWeight: 600, marginBottom: 6 }}>Distribución ideal</div><div style={{ fontSize: 11, color: T.dim, lineHeight: 1.6 }}><div>Necesidades (50%): {Math.round(totalInc * 0.5).toLocaleString("es-ES")}€</div><div>Deseos (30%): {Math.round(totalInc * 0.3).toLocaleString("es-ES")}€</div><div style={{ color: T.teal, fontWeight: 600 }}>Ahorro (20%): {Math.round(totalInc * 0.2).toLocaleString("es-ES")}€</div></div></div></Card>
    </div>
  );
}
      ...
      <div className="fu" style={{ width: "100%", maxWidth: 700 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
          <div onClick={() => onSelect("business")} style={{ background: T.navyMid, border: \`2px solid \${T.amber}44\`, borderRadius: 24, padding: 32, cursor: "pointer", transition: "all .25s", textAlign: "center" }} onMouseEnter={(e) => { e.currentTarget.style.borderColor = T.amber; e.currentTarget.style.transform = "translateY(-4px)"; }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = T.amber + "44"; e.currentTarget.style.transform = "translateY(0)"; }}>
            <div style={{ fontSize: 52, marginBottom: 14 }}>💼</div><div className="serif" style={{ fontSize: 22, color: T.amber, marginBottom: 10 }}>Autónomo / Pyme</div><div style={{ color: T.dim, fontSize: 13, lineHeight: 1.7 }}>Facturación con IVA y IRPF, modelos AEAT, gastos deducibles</div>
          </div>
          <div onClick={() => onSelect("family")} style={{ background: T.navyMid, border: \`2px solid \${T.teal}44\`, borderRadius: 24, padding: 32, cursor: "pointer", transition: "all .25s", textAlign: "center" }} onMouseEnter={(e) => { e.currentTarget.style.borderColor = T.teal; e.currentTarget.style.transform = "translateY(-4px)"; }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = T.teal + "44"; e.currentTarget.style.transform = "translateY(0)"; }}>
            <div style={{ fontSize: 52, marginBottom: 14 }}>🏠</div><div className="serif" style={{ fontSize: 22, color: T.teal, marginBottom: 10 }}>Personal / Familia</div><div style={{ color: T.dim, fontSize: 13, lineHeight: 1.7 }}>Control presupuesto, análisis ahorro, metas familiares</div>
          </div>
        </div>
      </div>
    </div>
...
export default function App() {
  ...
  const nav = appMode === "family" ? [{ id: "dashboard", label: "Resumen", icon: "🏠" }] : [{ id: "dashboard", label: "Dashboard", icon: "⚡" }];
      ...
      <div className="app-shell">
        ...
        <div className={\`sidebar \${sidebarOpen ? "open" : ""}\`}>
          ...
          <div style={{ padding: "12px" }}>
            <Btn full onClick={() => { setAppMode(null); setScreen("dashboard"); setSidebarOpen(false); }} variant="outline" style={{ fontSize: 12 }}>🔄 Cambiar modo</Btn>
          </div>
        </div>
        ...
        <div className="main-content">
          ...
          {screen === "dashboard" && (appMode === "business" ? <BusinessDashboard session={session} showToast={showToast} /> : <FamilyDashboard session={session} showToast={showToast} />)}
        </div>
      </div>
...
FINALEOF
echo "✅ App.jsx actualizado completamente"